...
export default class VelocityControl extends L.Control {
    ...
    _onMouseMove(e: L.LeafletMouseEvent) {
      var self = this;
      var pos = this._map.containerPointToLatLng(
        L.point(e.containerPoint.x, e.containerPoint.y)
      );
      var gridValue = this.options.leafletVelocity.windy.grid.get(
        pos.lng,
        pos.lat
      );
      var htmlOut = "";
...
export const ExtendedLControl = Object.assign(L.Control, { Velocity: L.Control.extend(new VelocityControl()) })
export const extendedLControl = Object.assign(L.control, { velocity: (options: VelocityControlOptions)=>new ExtendedLControl.Velocity(options) })
